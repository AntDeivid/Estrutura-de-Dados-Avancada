...
int Tree::_min_key(Node *node) { // TODO
    ...
    int min = node->key, leftMin, rightMin;

    if (node->left != nullptr) {
        leftMin = _min_key(node->left); 
        if (leftMin < min) { min = leftMin; }
    }

    if (node->right != nullptr) {
        rightMin = _min_key(node->right); 
...
int Tree::_um_filho(Node *node) { // TODO
    ...
    else if (node->left == nullptr || node->right == nullptr) { return 1 + _um_filho(node->left) + _um_filho(node->right); }
